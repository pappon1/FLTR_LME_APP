...
class _ReceivedMessagesTabState extends State<ReceivedMessagesTab> {
          ...
          builder: (context, provider, _) {
            ...
            return Container(
              ...
              child: Row(
                ...
                children: [
                  Text('$unreadCount Unread', style: const TextStyle(color: Colors.red, fontWeight: FontWeight.bold)),
                  TextButton.icon(
                   ...
                   child: SingleChildScrollView(
                     ...
                     child: Container(
                       ...
                       child: Column(
                         mainAxisAlignment: MainAxisAlignment.center,
                         children: [
                           FaIcon(_getIconForType(type), size: 48, color: Colors.grey[200]),
                           const SizedBox(height: 16),
                           Text('No active ${type}s', style: const TextStyle(color: Colors.grey)),
                         ],
                       ),
                     ),
                   ),
  ...
  IconData _getIconForType(String type) {
    switch (type) {
      case 'registration': return FontAwesomeIcons.download;
      case 'purchase': return FontAwesomeIcons.creditCard; 
      case 'message': return FontAwesomeIcons.solidCommentDots;
      default: return FontAwesomeIcons.bell;
    }
  }

  Widget _buildNotificationCard(BuildContext context, String docId, Map<String, dynamic> data, String type) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final timestamp = data['timestamp'] as Timestamp?;
    final dateStr = timestamp != null ? DateFormat('h:mm a, MMM d').format(timestamp.toDate()) : 'Just now';
    ...
    return Card(
      ...
      child: InkWell(
        ...
        onTap: () {
          // 1. Mark as Read
          if (!isRead) {
            Provider.of<AdminNotificationProvider>(context, listen: false).markAsRead(docId);
          }
            ...
            children: [
              ...
              Stack(
                children: [
                   CircleAvatar(
                    ...
                    child: userImage.isEmpty ? FaIcon(_getIconForType(type), size: 20, color: Colors.grey) : null,
                  ),
                  if (!isRead)
                    Positioned(
                      right: 0, 
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Expanded(child: Text(userName, style: TextStyle(fontWeight: !isRead ? FontWeight.w900 : FontWeight.bold, fontSize: 15), overflow: TextOverflow.ellipsis)),
                        Text(dateStr, style: TextStyle(fontSize: 11, color: !isRead ? AppTheme.primaryColor : Colors.grey[500], fontWeight: !isRead ? FontWeight.bold : FontWeight.normal)),
                      ],
                    ),
                    const SizedBox(height: 4),
                    
                    if (type == 'registration') ...[
                      Text('New App Download', style: TextStyle(color: Colors.blue[400], fontWeight: FontWeight.bold, fontSize: 13)),
                      if (device.isNotEmpty) Text('Device: $device', style: const TextStyle(color: Colors.grey, fontSize: 12)),
                    ],
                    ...
                    if (type == 'message') ...[
                       Text(message, style: TextStyle(color: isDark ? Colors.grey[300] : Colors.black87, fontWeight: !isRead ? FontWeight.w500 : FontWeight.normal), maxLines: 2, overflow: TextOverflow.ellipsis),
                    ],
                  ],
                ),
              ),
              ...
              if (type == 'message')
                 Padding(
                   ...
                   child: FaIcon(FontAwesomeIcons.chevronRight, size: 12, color: Colors.grey[400]),
                 )
            ],
          ),
        ),
