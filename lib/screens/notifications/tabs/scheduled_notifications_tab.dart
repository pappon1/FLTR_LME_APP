...
class ScheduledNotificationsTab extends StatefulWidget {
  ...
  @override
  State<ScheduledNotificationsTab> createState() => _ScheduledNotificationsTabState();
}
...
class _ScheduledNotificationsTabState extends State<ScheduledNotificationsTab> {
  ...
  Future<void> _cancelNotification(String id) async {
    final confirm = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        content: const Text('Are you sure you want to cancel this scheduled notification? It will be removed permanently.'),
        actions: [
               ...
               action: SnackBarAction(
                 ...
                 onPressed: () async {
                   if (data != null) {
                      ...
                      if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Restored!'), behavior: SnackBarBehavior.floating));
                   }
                 }
               ),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: Colors.transparent, // Keep transparent to blend with parent
        ...
        child: StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            if (snapshot.hasError) {
               return Stack(
                  children: [ListView(), Center(child: Text('Error: ${snapshot.error}'))]
               );
            }
                ...
                children: [
                  ...
                  Center(
                    child: Column(
                      ...
                      children: [
                        FaIcon(FontAwesomeIcons.clock, size: 64, color: Colors.grey[300]),
                        const SizedBox(height: 16),
                        Text('No Scheduled Notifications', style: AppTheme.heading3(context).copyWith(color: Colors.grey[500])),
                      ],
                    ),
                  ),
                ],
  ...
  Widget _buildSystemNotificationItem(BuildContext context, String id, Map<String, dynamic> data) {
    final isDark = Theme.of(context).brightness == Brightness.dark;
    final title = data['title'] ?? 'No Title';
    final message = data['message'] ?? 'No Message';
    final imageUrl = data['imageUrl'];
    final date = (data['scheduledAt'] as Timestamp?)?.toDate();
    final timeStr = date != null ? DateFormat('h:mm a • d MMM').format(date) : 'Unscheduled';
                  ...
                  child: Row(
                    children: [
                      Container(
                        ...
                        child: const Icon(Icons.handyman, size: 10, color: Colors.white),
                      ),
                      ...
                      errorWidget: (context, url, error) => Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 4),
                            Text('Error: $error', style: const TextStyle(fontSize: 10, color: Colors.grey))
                          ],
                        )
                      ),
